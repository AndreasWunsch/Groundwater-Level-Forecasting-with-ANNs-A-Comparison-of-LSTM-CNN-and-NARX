...
def gwmodel(ini,GLOBAL_SETTINGS,X_train, Y_train,X_stop, Y_stop):
    ...
    # model.add(ks.layers.Dropout(0.1)) #Dropout hier nur eventuell nötig
...
with tf.device("/cpu:0"):
    ...
    for pp in range(0,17): #loop over all wells
        ...
        f = open('./timelog_CNN_seq2seq_GWLt-1_'+Well_ID+'.txt', "a")
...
out_seqlength = {:d}\nbatchsize = {:d}\nrH = {:d}\nT = {:d}\nTsin = {:d}""".format(scores.NSE[0],scores.R2[0],
        ...
        pyplot.savefig(Well_ID+'_testset_CNN_seq2seq_GWLt-1.png', dpi=300)
        pyplot.show()
        
        # print log summary file
        f = open('./log_summary_CNN_seq2seq_GWLt-1_'+Well_ID+'.txt', "w")
        print("\nBEST:\n\n"+s+"\n", file = f)
