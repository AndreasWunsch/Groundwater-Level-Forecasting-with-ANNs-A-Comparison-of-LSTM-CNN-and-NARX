...
%     load(strcat(Well_ID,'_GW-Data')); 
    ...
    optimVars = [
        optimizableVariable('hiddenLayerSize', [1,20], 'Type', 'integer') %No. of hidden neurons
        optimizableVariable('InputrH',  [0,1], 'Type', 'integer') %Input
        optimizableVariable('InputT', [0,1], 'Type', 'integer') %Input
        optimizableVariable('InputTsin', [0,1], 'Type', 'integer') %Input
        optimizableVariable('ID_P', [1,52],'Type', 'integer') %Delay
        optimizableVariable('ID_rH', [1,52],'Type', 'integer') %Delay
        optimizableVariable('ID_T', [1,52],'Type', 'integer') %Delay
        optimizableVariable('ID_Tsin', [1,52],'Type', 'integer') %Delay
        optimizableVariable('FDmax', [1,52],'Type', 'integer')]; %Delay
    ...
    for ini = 1:inimax
        ...
        net.trainParam.max_fail = 5; %early stopping iterations
        %-------------------
    ...
    idxtestPI = idxtest(2:end);idxtestPI(end+1)=0;
    obsPI1 = Data1(idxtestPI,1);
    ...
    for i = 1:inimax
        ...
        errors.PI(i) = 1-(nansum(err.^2)/nansum(err_PI.^2));
    end
...
inimax = 5;
target = NaN(inimax,1);
for ini = 1:inimax %perform several runs due to dependency to random seed(ini)
    % rng(0)
    rng(ini)
    net = narxnet(1,feedbackDelays,hiddensz,architecture,optimizer); % Create a Nonlinear Autoregressive Network with External Input
    net.divideFcn = 'divideblock';  % Divide data blockwise (good for time series)
