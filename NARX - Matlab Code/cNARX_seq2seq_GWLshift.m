...
%     load(strcat(Well_ID,'_GW-Data')); 
    ...
    x5 = to_sequence_input(sequence_length,XTrain(:,5),size(XTrain,1));
    ...
    optimVars = [
        ...
        optimizableVariable('ID_GWLshift', [1,12],'Type', 'integer')
    ...
    for run = 1:runmax
        ...
        %% Calculations
        Dmax = nanmax(bestKonfig{pp,5:10})*sequence_length;
        ...
        for ini = 1:inimax %initialization ensemble
            ...
            delays_needed = max(bestKonfig{pp,5:10});
    ...
    str2 = string(bestKonfig{pp,1:10});
    ...
    print('-dpng','-r300',strcat(".\Forecast_NARX_seq2seq_GWLshift_Paper_",Well_ID))
    
    %% save logs
    fileID = fopen('log_summary_seq2seq_GWLshift_NARX_'+Well_ID+'.txt', 'w');
    fprintf(fileID,...
        "\nBEST:\n\nNSE = %.2f\nR²  = %.2f\nRMSE = %.2f\nrRMSE = %.2f\nBias = %.2f\nrBias = %.2f\nPI12 = %.2f\nPI12op = %.2f\n\n",...
        scores);
    ...
    %% save workspace
    toc
    save(strcat("Workspace_Bayesopt_seq2seq_GWLshift_",Well_ID));
end
...
function target = wrapNARXNet(x, y, varargin)
    ...
    for ini = 1:inimax %perform several runs due to dependency in random seed(ini)
        ...
        if inputs(5) == 1
            net.inputWeights{1,idx}.delays =  1:ip.Results.ID_GWLshift;
            net.inputs{idx,1}.name = 'GWLshift';idx = idx+1;
        end
