...
%     load(strcat(Well_ID,'_GW-Data')); 
    ...
    for run = 1:runmax
        ...
        %% Calculations
        Dmax = nanmax(bestKonfig{pp,6:11}); %maximum delay size
        ...
        for ini = 1:inimax %initialization ensemble
            ...
            if isfile(netfilepath)
                ...
                x5 = to_sequence_input(sequence_length,TrainingData_n.(6),Training_endindex);
            ...
            tempInput1 = TrainingData_n{Training_endindex-Dmax-sequence_length+1:Training_endindex,2:end}; %needed to fill the delays prior to testing
            tempTarget1 = TrainingData_n{Training_endindex-Dmax-sequence_length+1:Training_endindex,1}; %needed to fill the delays prior to testing
            ...
            xt5 = to_sequence_input(sequence_length,TestingInput(:,5),endidx);
            ...
            delays_needed = max(bestKonfig{pp,6:11});
            ...
            [TestingInput,ID_ini,FD_ini,TestingTarget,~,~] = preparets(net,TestingInput,{},TestingTarget);%prepare data
    ...
    for run = 1:runmax  
        ...
        Forecast_Errors(7,run) = 1-(nansum(err.^2)/nansum(err_PI12op.^2));           % [ ] PI12op, Persistency Index 12 week / 3 month forecast
    end
    time2 = datetime('now');
    ...
    %obs Data
    plot(TrainingData.Date(idx_forecaststart:end),TrainingData.GWL(idx_forecaststart:end),'b','LineWidth',1.5), hold on
    
    %sim ensemble member
    for i = 1:size(TestResults,2)
        plot(TestResults.Date,TestResults.(i),'Color',[1 0 0 0.1],'LineWidth',1.5,'HandleVisibility','off')
    end
    ...
    str1_2 = {["NSE [ ]" ],["R² [ ]"],["RMSE [m]" ],["rRMSE [%]"],["Bias [m]" ],["rBias [%]"],["PI12op [ ]"],[" "]};
    str2_2 = {sprintf('%0.2f\n%0.2f\n%0.2f\n%0.2f\n%0.2f\n%0.2f\n%0.2f\n',...
        ...
        median(Forecast_Errors(7,:)))};
    annotation('textbox',dim,'String',[str1_2,str1]);
    annotation('textbox',dim+[0.06,0,0,0],'String',[str2_2,str2],'EdgeColor','none');
    
    print('-dpng','-r300',strcat(".\Forecast_NARX_seq2seq_12steps_",Well_ID))
    %%
    % close all
    save(strcat(".\Forecast_NARX_seq2seq_12steps_Workspace_",Well_ID));
end
