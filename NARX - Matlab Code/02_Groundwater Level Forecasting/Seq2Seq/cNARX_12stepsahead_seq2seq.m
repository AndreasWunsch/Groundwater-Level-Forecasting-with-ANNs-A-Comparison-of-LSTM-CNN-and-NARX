...
%     load(strcat(Well_ID,'_GW-Data')); 
    ...
    inputs = [1 bestKonfig.InputrH(pp) bestKonfig.InputT(pp) bestKonfig.InputTsin(pp)];
    ...
    for run = 1:runmax
        ...
        %% Calculations
        Dmax = nanmax(bestKonfig{pp,6:10})*sequence_length;
        ...
        for ini = 1:inimax %initialization ensemble
            ...
            delays_needed = max(bestKonfig{pp,6:10});
    ...
    dim = [0.8615,0.27,0.10,0.53];
    str1 = ["HiddenSize","rH","T","Tsin","ID P","ID rH","ID T","ID Tsin","FD"];
    str2 = string(bestKonfig{pp,2:10});
